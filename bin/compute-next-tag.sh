...
#   released, the release counter restarts at 0 (`v1.31.7-0`)
# - otherwise the counter is incremented (`v1.31.7-1`), but only if something
...
version="$(sed -nE 's|^nginx_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"
...
# The version values may or may not carry a leading `v` (e.g. `1.31.6`),
